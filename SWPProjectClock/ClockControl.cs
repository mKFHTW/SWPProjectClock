    ...
    public partial class ClockControl : UserControl
    {
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (ShowSecondhand)
                ...
                pe.Graphics.DrawLine(Pens.Black, origin, PointOnCircle(radius, second * 6f, origin));

            //Draw only if ShowMinuteHand is true 
            if (ShowMinuteHand)
                //Draw the minute hand 
                pe.Graphics.DrawLine(Pens.Black, origin, PointOnCircle(radius * 0.75f, minute * 6f, origin));

            //Draw only if ShowHourHand is true 
            if (ShowHourHand)
                //Draw the hour hand 
                pe.Graphics.DrawLine(Pens.Black, origin, PointOnCircle(radius * 0.50f, hour * 30f, origin));
